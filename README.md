...
- 2025년 권장 네이티브 통합 방식 사용
- 환경별 Supabase 클라이언트 분리:
  - Client Component용 (`useClerkSupabaseClient` hook)
  - Server Component/Server Action용 (`createClerkSupabaseClient`)
...
4. **Clerk domain**이 표시되면 복사 (예: `https://your-app-12.clerk.accounts.dev`)
...
- Clerk 세션 토큰이 Supabase에서 자동으로 검증됩니다
- `auth.jwt()->>'sub'`로 Clerk user ID에 접근할 수 있습니다
- RLS 정책에서 Clerk user ID를 사용하여 데이터 접근을 제어할 수 있습니다

자세한 내용은 [Clerk 공식 통합 가이드](https://clerk.com/docs/guides/development/integrations/databases/supabase)를 참고하세요.
...
**선택사항: Supabase 공식 문서 예시 테이블 생성**

`/instruments` 테스트 페이지를 사용하려면 다음 SQL을 실행하세요:
...
- `/instruments`: Supabase 데이터 조회 테스트 (공식 문서 예시)
...
- `app/layout.tsx`의 `ClerkProvider`에서 `koKR` locale 사용
- Tailwind CSS 4 호환성을 위한 `appearance.cssLayerName: 'clerk'` 설정
- HTML 문서 언어: `lang="ko"`

**적용 범위:**

- 모든 Clerk 컴포넌트 (SignIn, SignUp, UserButton 등)가 한국어로 표시됩니다
- 로그인/회원가입 모달, 폼 레이블, 에러 메시지 등이 모두 한국어로 표시됩니다

**커스텀 로컬라이제이션:**

특정 메시지를 브랜드에 맞게 커스터마이징하려면 `lib/clerk/localization.ts` 파일을 참고하세요:

```tsx
// app/layout.tsx
import { customKoreanLocalization } from "@/lib/clerk/localization";

<ClerkProvider localization={customKoreanLocalization}>...</ClerkProvider>;
```

자세한 내용은 [Clerk 공식 문서](https://clerk.com/docs/guides/customizing-clerk/localization)를 참고하세요.

> **참고**: 로컬라이제이션은 Clerk 컴포넌트의 텍스트만 변경합니다. Clerk Account Portal은 여전히 영어로 표시됩니다.
...
이 변수들이 설정되지 않으면 명확한 에러 메시지가 표시됩니다.
...
프로젝트는 환경별로 Supabase 클라이언트를 분리하여 제공합니다:
...
> **주의**: `getServiceRoleClient()`는 서버 사이드에서만 사용하고, 클라이언트에 노출하면 안 됩니다.
...
│   │   ├── server.ts          # Server Component/Action용 (createClerkSupabaseClient)
